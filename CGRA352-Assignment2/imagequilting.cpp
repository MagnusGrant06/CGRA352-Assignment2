...
void ImageQuilting::run_quilting() {
	canvas = cv::Mat(100, 500, CV_8UC3, cv::Scalar(0,0,0));
...
void ImageQuilting::h_quilt(cv::Mat left, cv::Mat right, int overlap_size) {
	...
	cv::Mat error(left.rows, overlap_size, CV_32FC1);
	//compute pixel wise difference in overlapping area

	cv::Mat acc_error(left.rows, overlap_size, CV_32FC1);
	std::vector<int> trace(left.rows,-1);
	...
	for (int r = acc_error.rows-2; r > -1; r--) {
		...
		if(col != 0) left_up = acc_error.at<float>(r, col - 1);
		if(col != acc_error.cols-1) right_up = acc_error.at<float>(r, col + 1);
	...
	cv::Mat mask(left.rows, patch_size, CV_8U,cv::Scalar(0));
	...
	synthesis = right;
	cv::imshow("sldjkas", mask);
	cv::waitKey(0);
	right.copyTo(canvas(cv::Rect(80*temp,0,patch_size,patch_size)), mask);
}
